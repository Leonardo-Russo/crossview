...
class CombinedPairedImagesDataset(Dataset):
    def __init__(self, filenames_panos, filenames_cutouts, transform_aerial=None, transform_panos=None, transform_cutouts=None):
        ...
        self.filenames_cutouts = filenames_cutouts
        self.transform_aerial = transform_aerial
        self.transform_panos = transform_panos
        self.transform_cutouts = transform_cutouts
        self.filenames = self.filenames_panos + self.filenames_cutouts

    def __len__(self):
        return len(self.filenames)

    def __getitem__(self, idx):
        if idx < len(self.filenames_panos):
            ground_img_path, aerial_img_path = self.filenames_panos[idx]
            ground_image = Image.open(ground_img_path).convert('RGB')
            if self.transform_panos:
                ground_image = self.transform_panos(ground_image)
        else:
            ground_img_path, aerial_img_path = self.filenames_cutouts[idx - len(self.filenames_panos)]
            ground_image = Image.open(ground_img_path).convert('RGB')
            if self.transform_cutouts:
                ground_image = self.transform_cutouts(ground_image)
